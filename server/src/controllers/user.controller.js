...
export const getSuggestedUsers = catchAsyncError(async (req, res) => {
  ...
  if (!users) throw new ErrorHandler(404, "Suggested Users are not available currently.")
...
export const updateProfile = catchAsyncError(async (req, res, next) => {
  ...
  const { isUnchanged, isNameSame, isFullNameSame, isBioSame, isGenderSame, isDobSame } =
    profileChanges(user, { name, fullName, bio, gender, dob: birthday }, !!profilePicture)

  if (isUnchanged) return handleSuccessResponse(res, 200, "No changes were made", { user })

  if (profilePicture) {
    if (user.profilePicturePublicId) await deleteFile(user.profilePicturePublicId)

    const { url, publicId } = await uploadFile(profilePicture, `avatar_${uuidv4()}`, "avatars")
    if (!url || !publicId) return next(new ErrorHandler(500, "Failed to upload profile picture"))
...
export const deleteProfilePicture = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    await user.save()
    return handleSuccessResponse(res, 200, result || "Profile picture removed.", { user })
  } catch (error) {
    throw new ErrorHandler(500, error.message || "Failed to delete profile picture")
  }
})
...
export const blockUnblockUser = catchAsyncError(async (req, res) => {
  ...
  if (!blockedId) return handleErrorResponse(res, 400, " user ID is required for this opperation")

  const blockedUser = await User.findById(blockedId)
  if (!blockedUser) return handleErrorResponse(res, 404, "User not found")

  if (blockedId === blockerId) return handleErrorResponse(res, 400, "You cannot block yourself")
