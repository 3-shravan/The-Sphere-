...
import { checkExistingUsers, handleUnverifiedUser } from "../services/user.services.js"
...
export const register = catchAsyncError(async (req, res) => {
  ...
  if (isExistUnverifiedUser) verificationCode = await handleUnverifiedUser(isExistUnverifiedUser)
  else {
    // Create new
...
export const verifyOTP = catchAsyncError(async (req, res, next) => {
  ...
  if (phone && !validatePhoneNo(phone)) return next(new ErrorHandler(400, "Invalid phone number"))
  ...
  if (verificationcode !== Number(otp)) throw new ErrorHandler(400, "Invalid OTP")

  //check if the provided OTP is expired one
  const expireTime = new Date(user.verificationCodeExpire).getTime()
  if (expireTime < Date.now()) throw new ErrorHandler(400, "OTP Expired. Please request a new one.")
...
export const forgetPassword = catchAsyncError(async (req, res) => {
  ...
  const user = await User.findOne(query)
  if (!user) throw new ErrorHandler(404, `No user is registered with ${email || phone}`)
  ...
  try {
    if (email) {
      ...
      return handleSuccessResponse(res, 200, `Reset password link is sent to ${email}`)
    }
    if (phone) {
...
export const verifyResetPasswordOTP = catchAsyncError(async (req, res) => {
...
export const resetPasswordWithEmailToken = catchAsyncError(async (req, res) => {
  ...
  if (newPassword !== confirmPassword) throw new ErrorHandler(400, "Passwords do not match")
  ...
  if (isMatch) throw new ErrorHandler(400, "Previously used password. Use a new one.")
...
export const resetPasswordWithPhone = catchAsyncError(async (req, res) => {
  ...
  if (!user) throw new ErrorHandler(400, "Unauthorized or invalid phone-based reset")
...
export const usernameAvailability = catchAsyncError(async (req, res) => {
  ...
  if (validationError) return res.json({ available: false, message: validationError })
  ...
  if (existingUser) return res.json({ available: false, message: "username already taken" })
