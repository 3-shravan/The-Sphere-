...
export const createGroupChat = catchAsyncError(async (req, res) => {
  ...
  if (groupPicture)
    media = await uploadFile(groupPicture, `group_picture${uuidv4()}`, "Group Picture")
...
export const updateGroup = catchAsyncError(async (req, res) => {
  ...
  if (!group.isGroupChat) throw new ErrorHandler(400, "This is not a group chat")
  if (group.groupName === groupName && group.groupDescription === groupDescription)
    throw new ErrorHandler(400, "No made no changes")

  if (!group.users.includes(req.userId))
    throw new ErrorHandler(403, "You are not a member of this group.Can't make changes")
...
export const changeGroupPicture = catchAsyncError(async (req, res) => {
  ...
  if (!group.users.includes(req.userId))
    throw new ErrorHandler(403, "You are not an member of this group. Can't change the picture.")
...
export const removeFromGroup = catchAsyncError(async (req, res) => {
  ...
  if (!userId || !chatId) throw new ErrorHandler(400, "UserId and chatId are required")
  ...
  if (!group.users.includes(userId)) throw new ErrorHandler(400, "User already not a group member")
  if (group.groupCreatedBy.toString() === userId.toString())
    throw new ErrorHandler(400, "The creator of the group can't be removed")
  if (!group.admins.includes(req.userId) && req.userId.toString() !== userId.toString())
  ...
  return handleSuccessResponse(res, 200, "User removed from group successfully")
})
...
export const UpdateAdmin = catchAsyncError(async (req, res) => {
  ...
  const admins = [
    ...
    ...group.admins.filter((admin) => admin._id.toString() !== group.groupCreatedBy._id.toString()),
  ]
