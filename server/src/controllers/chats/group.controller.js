...
export const createGroupChat = catchAsyncError(async (req, res) => {
  ...
  if (users.length < 2 || users.length > 49)
    throw new ApiError(400, "Group must have 2 to 50 members");
  ...
  if (!groupChat) throw new ApiError(500, "Failed to create group chat");
...
export const updateGroup = catchAsyncError(async (req, res) => {
  ...
  if (!group.isGroupChat) throw new ApiError(400, "This is not a group chat");
  if (
    group.groupName === groupName &&
    group.groupDescription === groupDescription
  )
    throw new ApiError(400, "No made no changes");
...
export const changeGroupPicture = catchAsyncError(async (req, res) => {
  ...
  const groupPicture = req.file;
  if (!groupPicture) throw new ApiError(400, "Group picture is required");
...
export const addToGroup = catchAsyncError(async (req, res) => {
  ...
  if (group.users.length + users.length > 50)
    throw new ApiError(400, "Group can have only 50 members");
...
export const removeFromGroup = catchAsyncError(async (req, res) => {
  ...
  if (!userId || !chatId)
    throw new ApiError(400, "UserId and chatId are required");
  ...
  if (!group.users.includes(userId))
    throw new ApiError(400, "User already not a group member");
  if (group.groupCreatedBy.toString() === userId.toString())
    throw new ApiError(400, "The creator of the group can't be removed");
  if (
    !group.admins.includes(req.userId) &&
    req.userId.toString() !== userId.toString()
  )
    throw new ApiError(403, "Only admins can remove users from the group");
  if (group.admins.includes(userId))
    throw new ApiError(400, "An admin of the group can't be removed");
...
export const UpdateAdmin = catchAsyncError(async (req, res) => {
  ...
  if (group.groupCreatedBy.toString() === userId.toString())
    throw new ApiError(400, "Group Creator can't be removed as admin");
