...
export const sendMessage = catchAsyncError(async (req, res, next) => {
  ...
  if (!receiverId || (!content && !image)) return next(new ErrorHandler(400, "Invalid data"))
  ...
  if (chat) {
    ...
    if (chat.isGroupChat) {
      ...
      if (!isMember) return next(new ErrorHandler(403, "You are not a member of this group"))

      const message = await createAndSaveMessage(chat._id, senderId, content, media)
  ...
  const newMessage = await createAndSaveMessage(chat._id, senderId, content, media)
  return handleSuccessResponse(res, 200, "Message sent successfully", {
