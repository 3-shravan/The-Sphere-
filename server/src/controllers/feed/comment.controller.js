...
export const getPostComments = catchAsyncError(async (req, res, next) => {
  ...
  const comments = await Comment.find({ post: postId })
    ...
    .sort({ createdAt: -1 });
  if (!comments) return next(new ApiError(404, "No comments found"));
...
export const commentPost = catchAsyncError(async (req, res, next) => {
  ...
  if (!comment) throw new ApiError(404, "Please enter a comment");

  const post = await Post.findById(postId);
  if (!post) throw new ApiError(404, "Post not found");

  // if (await userBLocked(commentedBy, post.author)) {
  //   return next(new ApiError(403, "You cannot comment on this post"));
  ...
  if (!newComment) return next(new ApiError(500, "Failed to create comment"));
...
export const deleteComment = catchAsyncError(async (req, res, next) => {
  ...
  if (comment.author.toString() !== authorID.toString())
    return next(new ApiError(400, "Unautorize to delete this comment."));
