...
export const sendMessage = catchAsyncError(async (req, res, next) => {
   ...
   if (!receiverId || (!content && !media)) return next(new ErrorHandler(400, 'Invalid data'));

   let chat = await Chat.findOne({ _id: receiverId })

   if (chat && chat.isGroupChat) {
      if (!chat.users.includes(req.user._id))
         return next(new ErrorHandler(400, 'You are not a member of this group chat'));
   ...
   if (isBlocked) {
      if (isBlocked.blockerId.toString() === req.user._id) return next(new ErrorHandler(400, 'You blocked this user'));
      if (isBlocked.blockerId.toString() === receiverId) return next(new ErrorHandler(400, 'This user blocked you'));
   }
