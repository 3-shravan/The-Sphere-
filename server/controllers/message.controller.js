...
//   $or: [
//     { blockerId: req.user._id, blockedId: id },
...
export const sendMessage = catchAsyncError(async (req, res, next) => {
  ...
  let media = null;
  if (image) {
    const publicId = `message_${uuidv4()}`;
    media = await processImageUpload(image, publicId, "messages", next);
  }
...
export const deleteMessage = catchAsyncError(async (req, res, next) => {
  ...
  if (!messageId) return next(new ErrorHandler(400, "messageId missing"));
